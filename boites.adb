...
package body boites is
	-- Point d'ancrage utilisé lors du dessin (ce qui permet de ne travailler qu'avec des 
	-- coordonnees relatives a ce point)
	AncreX, AncreY : Float;

	-- Type de donnees representant une demi-boite
	...
	begin
		...
		while DB.largeur - resultat*DB.longueur_queue < 2*(DB.epaisseur+Natural(MIN_MARGE)) loop
			resultat := resultat - 1;
		end loop;
	...
	begin
		...
		while DB.hauteur - resultat*DB.longueur_queue < DB.epaisseur+Natural(MIN_MARGE) loop
			resultat := resultat - 1;
		end loop;
