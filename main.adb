...
procedure main is
	...
	begin    
		while Arg < Argument_Count loop
			...
			elsif Argument(Arg) = "--couleur" then
				Put_Line("! La valeur de l'option --couleur n'est pas verifiee.");
				COULEUR := To_Unbounded_String(Argument(Arg+1));
			elsif Argument(Arg) = "--epaisseur" then
				Put_Line("! La valeur de l'option --epaisseur n'est pas verifiee.");
				EPAISSEUR := To_Unbounded_String(Argument(Arg+1));
			elsif Argument(Arg) = "--min-marge" then
				MIN_MARGE := Float'Value(Argument(Arg+1));
			elsif Argument(Arg) = "--marge-svg" then
				MARGE_SVG := Float'Value(Argument(Arg+1));
			else
				Put_Line("Option non reconnue : " & Argument(Arg));
				raise Erreur_Parametres_Exception;
			end if;
			-- on passe l'option et la valeur qui suit
			Arg := Arg + 2;
		end loop;
...
begin
	begin
		...
		-- Si pas d'exception jusqu'ici, alors tout s'est bien passe a priori
